...
def yedek_soru(ders, soru): return f"İçerik modülü yüklenemediği için {ders} dersi '{konu}' sorusu çözülemiyor."
...
# TÜM GİRİNTİ HATALARI BU KISIMDA DÜZELTİLMİŞTİR (50. satır civarı)
try:
...
def user_login(username, password):
    if not st.session_state['user_login_allowed']:
        st.error("Üye girişi şu anda bakımdadır. Lütfen daha sonra tekrar deneyin.") # Düzeltildi
        return
...
def toggle_admin_login_panel():
    st.session_state['show_admin_login'] = not st.session_state['show_admin_
